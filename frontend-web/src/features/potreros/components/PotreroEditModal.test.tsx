...
const propiedad = { id: 'p-1', nombre: 'La Esperanza', version: 1 } as Propiedad
...
describe('PotreroEditModal', () => {
  it('permite quitar asociaciones y valores opcionales explícitamente', async () => {
    ...
    render(<QueryClientProvider client={client}><PotreroEditModal potrero={potrero} properties={[propiedad]} grasses={[{ id: 'g-1', codigo: 'BRACHIARIA', nombre: 'Brachiaria' }]} loading={false} error={null} onClose={vi.fn()} onSubmit={onSubmit} onReload={vi.fn()} /></QueryClientProvider>)
  ...
  it('calcula la capacidad recomendada y no permite editarla manualmente', async () => {
    const client = new QueryClient({ defaultOptions: { queries: { retry: false } } })
    render(<QueryClientProvider client={client}><PotreroEditModal potrero={potrero} properties={[propiedad]} grasses={[{ id: 'g-1', codigo: 'BRACHIARIA', nombre: 'Brachiaria' }]} loading={false} error={null} onClose={vi.fn()} onSubmit={vi.fn()} onReload={vi.fn()} /></QueryClientProvider>)

    const capacidad = screen.getByLabelText('Capacidad recomendada (UA)')
    expect(capacidad).toHaveValue(18)
    expect(capacidad).toHaveAttribute('readonly')
  })
})
