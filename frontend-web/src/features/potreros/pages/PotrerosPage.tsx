...
export function PotrerosPage() {
  ...
  return <div className="page-stack">
    ...
    <Card>{paddocks.isPending && <LoadingState message="Consultando potreros…" />}{paddocks.data?.length === 0 && <EmptyState title="No hay potreros" description="Crea una propiedad y registra su primer potrero." />}
      {paddocks.data && paddocks.data.length > 0 && <><div className="table-wrapper desktop-only"><table><caption className="visually-hidden">Potreros registrados</caption><thead><tr><th scope="col">Potrero</th><th scope="col">Propiedad</th><th scope="col">Superficie</th><th scope="col">Capacidad</th><th scope="col">Agua</th><th scope="col">Estado</th></tr></thead><tbody>{paddocks.data.map((item) => <tr key={item.id}><td><strong>{item.codigo}</strong><span className="table-secondary">{item.nombre}</span></td><td>{catalogs.data?.properties.find((property) => property.id === item.propiedadId)?.nombre ?? item.propiedadId}</td><td>{item.superficieHa ? `${item.superficieHa} ha` : '—'}</td><td>{item.capacidadUa ?? '—'}</td><td>{item.tieneAgua ? 'Sí' : 'No'}</td><td><select aria-label={`Estado de ${item.nombre}`} value={item.estado} disabled={changeState.isPending} onChange={(event) => setStateTarget({ potrero: item, estado: event.target.value as Potrero['estado'] })}><option>DISPONIBLE</option><option>OCUPADO</option><option>DESCANSO</option><option>MANTENIMIENTO</option></select></td></tr>)}</tbody></table></div><div className="mobile-only"><div className="mobile-entity-list">{paddocks.data.map((item) => <MobileEntityCard key={item.id} title={`${item.codigo} · ${item.nombre}`} status={<span className="status-badge">{item.estado}</span>} subtitle={catalogs.data?.properties.find((property) => property.id === item.propiedadId)?.nombre ?? item.propiedadId} metadata={<><span>{item.superficieHa ? `${item.superficieHa} ha` : 'Superficie no registrada'} · {item.capacidadUa ?? '—'} UA</span><span>Agua: {item.tieneAgua ? 'Sí' : 'No'}</span></>} action={<select aria-label={`Cambiar estado de ${item.nombre}`} value={item.estado} disabled={changeState.isPending} onChange={(event) => setStateTarget({ potrero: item, estado: event.target.value as Potrero['estado'] })}><option>DISPONIBLE</option><option>OCUPADO</option><option>DESCANSO</option><option>MANTENIMIENTO</option></select>} />)}</div></div></>}
    </Card>
    <ConfirmDialog
