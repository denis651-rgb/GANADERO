...
export function AuditoriaPage() {
  ...
  const query = useQuery({
    queryKey: ['auditoria', { modulo, accion, entidad, propiedadId, correlationId, desde, hasta, page, size }],
    queryFn: () => listAuditoria({ modulo, accion, entidad, propiedadId, correlationId, desde: desde || undefined, hasta: hasta || undefined, page, size }),
    placeholderData: keepPreviousData,
  })
  const propiedades = useQuery({ queryKey: ['auditoria-propiedades'], queryFn: listPropiedades })
  const error = query.error ?? propiedades.error
  ...
  return <div className="page-stack">
    ...
    <Card>
      ...
      <div className="animal-filters">
        <select aria-label="Filtrar por módulo" value={modulo} onChange={(event) => { setModulo(event.target.value); setPage(0) }}><option value="">Todos los módulos</option>{['ANIMALES', 'PROPIEDADES', 'POTREROS', 'LOTES', 'MOVIMIENTOS', 'PESAJE', 'REPRODUCCION', 'SANIDAD', 'VENTAS', 'ARCHIVOS', 'AUDITORIA'].map((value) => <option key={value}>{value}</option>)}</select>
        <select aria-label="Filtrar por acción" value={accion} onChange={(event) => { setAccion(event.target.value); setPage(0) }}><option value="">Todas las acciones</option>{['CREAR', 'ACTUALIZAR', 'ELIMINAR', 'CERRAR', 'ASIGNAR', 'CONFIRMAR', 'ANULAR', 'RESOLVER_QR'].map((value) => <option key={value}>{value}</option>)}</select>
        <select aria-label="Filtrar por entidad" value={entidad} onChange={(event) => { setEntidad(event.target.value); setPage(0) }}><option value="">Toda entidad</option>{['ANIMAL', 'IDENTIFICADOR', 'PARENTESCO', 'LOTE', 'MOVIMIENTO', 'PESAJE', 'DOCUMENTO', 'PROPIEDAD', 'POTRERO', 'VENTA'].map((value) => <option key={value}>{value}</option>)}</select>
      ...
      {query.data && query.data.content.length > 0 && <>
        <div className="table-wrapper desktop-only"><table><caption className="visually-hidden">Registros de auditoría</caption><thead><tr><th scope="col">Fecha</th><th scope="col">Módulo</th><th scope="col">Acción</th><th scope="col">Entidad</th><th scope="col">Resultado</th><th scope="col">Acciones</th></tr></thead><tbody>{query.data.content.map((item) => {
          const isOpen = expanded === item.id
          return <Fragment key={item.id}>
            <tr><td>{new Date(item.createdAt).toLocaleString('es-BO')}</td><td>{item.modulo}</td><td><span className="status-badge">{item.accion}</span></td><td>{item.entidad}</td><td><span className="status-badge">{item.resultado}</span></td>
            ...
            {isOpen && <tr className="selected-row"><td colSpan={6}><div className="permission-section">
