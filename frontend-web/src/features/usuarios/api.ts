...
export async function createUsuario(input: { usuarioId: string; nombres: string; apellidos: string; telefono?: string; cargo?: string; accesoTodasPropiedades: boolean; roles: string[] }) { return (await http.post<ApiResponse<Miembro>>('/api/v1/usuarios', input, { headers: { 'Idempotency-Key': crypto.randomUUID() } })).data.data }
