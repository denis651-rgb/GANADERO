...
export function LoteDetailPage() {
  ...
  return <div className="page-stack">
    ...
    <Card>
      ...
      {miembros.data && miembros.data.length > 0 && <div className="table-wrapper"><table><thead><tr>{value.estado === 'ACTIVO' && <th></th>}<th>Animal</th><th>Ingreso</th><th>Motivo</th><th></th></tr></thead><tbody>{miembros.data.map((item) => {
    ...
    <Modal open={showAdd} title="Agregar animales al lote" onClose={() => { if (!add.isPending) setShowAdd(false) }} wide>
      <div className="page-stack">
        <div className="filter-heading"><span className="search-box"><Search size={18} /><input value={addSearch} onChange={(event) => { setAddSearch(event.target.value); setAddSelected(new Set()) }} placeholder="Buscar por código o nombre" /></span><span className="muted">{addSelected.size} seleccionado(s)</span></div>
