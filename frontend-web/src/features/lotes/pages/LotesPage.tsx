...
export function LotesPage() {
  ...
  return <div className="page-stack">
    ...
    <Card>
      <div className="filter-heading"><span className="search-box"><Search size={18} aria-hidden="true" /><input type="search" aria-label="Buscar lotes" value={search} onChange={(event) => { setSearch(event.target.value); setPage(0) }} placeholder="Buscar por código o nombre…" /></span>
      ...
      {query.data && query.data.content.length > 0 && <>
        <div className="table-wrapper desktop-only"><table><caption className="visually-hidden">Lotes que coinciden con los filtros</caption><thead><tr><th scope="col">Código</th><th scope="col">Nombre</th><th scope="col">Propiedad</th><th scope="col">Ocupación</th><th scope="col">Estado</th><th scope="col">Apertura</th><th scope="col">Cierre</th><th scope="col">Acciones</th></tr></thead><tbody>{query.data.content.map((lote) => <tr key={lote.id}>
          ...
          <td><span className="status-badge">{lote.estado}</span></td><td>{formatDate(lote.fechaApertura)}</td><td>{lote.fechaCierre ? formatDate(lote.fechaCierre) : '—'}</td>
        ...
        </tr>)}</tbody></table></div><div className="mobile-only"><div className="mobile-entity-list">{query.data.content.map((lote) => <MobileEntityCard key={lote.id} title={`${lote.codigo} · ${lote.nombre}`} status={<span className="status-badge">{lote.estado}</span>} subtitle={propiedades.data?.find((item) => item.id === lote.propiedadId)?.nombre ?? 'Propiedad no disponible'} metadata={<><span>{lote.cantidadActual ?? 0} / {lote.cantidadMaxima ?? 'Sin límite'} animales{lote.cantidadMaxima != null ? ` · ${Math.max(0, lote.cantidadMaxima - lote.cantidadActual)} cupos disponibles` : ''}</span><span>Apertura: {formatDate(lote.fechaApertura)}</span>{lote.fechaCierre && <span>Cierre: {formatDate(lote.fechaCierre)}</span>}</>} action={<Link className="button button-ghost" to={`/lotes/${lote.id}`}>Ver lote →</Link>} />)}</div></div>
