...
vi.mock('@/features/compras/api', () => ({ getResumenCompraAnimal: vi.fn() }))
...
describe('MovimientoDetailModal', () => {
  it('resuelve el nombre de un animal vendido y oculta los datos técnicos', async () => {
    ...
    expect(screen.getByRole('link', { name: 'Ver venta de origen' })).toHaveAttribute('href', '/ventas?animalId=animal-vendido')
  })

  it('muestra el enlace a la compra de origen para un movimiento INGRESO_COMPRA', async () => {
    vi.mocked(getAnimal).mockResolvedValue({ id: 'animal-comprado', nombre: 'LB-02', codigo: 'ANI-000002' } as Awaited<ReturnType<typeof getAnimal>>)
    vi.mocked(getResumenCompraAnimal).mockResolvedValue({ id: 'compra-1', codigo: 'C-0001', fechaRecepcion: '2026-09-01T12:00:00-04:00', modalidad: 'POR_UNIDAD', moneda: 'BOB' })
    render(<QueryClientProvider client={new QueryClient({ defaultOptions: { queries: { retry: false } } })}>
      <MemoryRouter>
        <MovimientoDetailModal open onClose={vi.fn()} onValidar={vi.fn()} onConfirmar={vi.fn()} onAnular={vi.fn()} onRevertir={vi.fn()} onViewRelated={vi.fn()} pending={{}}
          movimiento={{ id: 'compra-mov', tipo: 'INGRESO_COMPRA', estado: 'CONFIRMADO', fechaMovimiento: '2026-09-01', version: 1 }}
          detalles={[{ id: 'detalle', animalId: 'animal-comprado', animalVersionEsperada: 1, estadoDespues: 'ACTIVO', estadoResultado: 'OK' }]}
          catalogs={{ animales: [], propiedades: [], potreros: [], lotes: [] }} />
      </MemoryRouter>
    </QueryClientProvider>)
    expect(await screen.findByRole('link', { name: /Ver compra de origen/ })).toHaveAttribute('href', '/compras/compra-1')
  })
})
