...
class StorageServiceTest{
 private static final byte[] PNG_1x1=Base64.getDecoder().decode("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg==");
 ...
 @Test void rejectsRenamedExecutable(){StorageService service=service();var file=new MockMultipartFile("file","avatar.png","image/png","MZ\u0000\u0000executable".getBytes());assertThatThrownBy(()->service.uploadAvatar(file)).isInstanceOfSatisfying(BusinessException.class,e->assertThat(e.code()).isEqualTo(ErrorCode.STORAGE_FILE_INVALID));}
 @Test void buildsTenantOwnedUuidPath(){CapturingClient client=new CapturingClient();CurrentUser user=user();StorageService service=service(client,user);var file=new MockMultipartFile("file","avatar.png","image/png",PNG_1x1);var result=service.uploadAvatar(file);assertThat(result.path()).startsWith("empresas/"+user.empresaId()+"/usuarios/"+user.userId()+"/avatar/").endsWith(".png");}
 private StorageService service(){return service(new CapturingClient(),user());}
 private StorageService service(SupabaseStorageClient client,CurrentUser user){AppProperties p=new AppProperties(new AppProperties.Bootstrap(false,""),new AppProperties.InternalJobs(false,""),new AppProperties.SystemStatus(false),"http://localhost",new AppProperties.Storage("bucket",1024,Duration.ofMinutes(5),List.of("image/png"),List.of("png")));return new StorageService(client,p,new UserContext(()->user),new ImagenValidador());}
