...
@RestController @RequestMapping("/api/v1/sanidad") public class PlanSanitarioController {
 ...
 @PutMapping("/planes/{plan}/items/{id}") ApiResponse<PlanSanitarioItem> actualizarItem(@PathVariable UUID plan,@PathVariable UUID id,@RequestParam long version,@Valid @RequestBody CrearPlanItemRequest b,HttpServletRequest r){return ok(service.actualizarItem(plan,id,b.command(),version),r);}
 @GetMapping("/planes/{plan}/items/{id}/versiones") ApiResponse<List<PlanSanitarioItem>> versiones(@PathVariable UUID plan,@PathVariable UUID id,HttpServletRequest r){return ok(service.versiones(id),r);}
 @GetMapping("/planes/{id}/conflictos") ApiResponse<List<PlanSanitario>> conflictos(@PathVariable UUID id,HttpServletRequest r){return ok(service.conflictosActivacion(id),r);}
