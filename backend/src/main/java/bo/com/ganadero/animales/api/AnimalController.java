...
import bo.com.ganadero.animales.domain.AnimalFilter;
...
public class AnimalController {
    ...
    ApiResponse<AnimalPageResponse> list(@RequestParam(required = false) EstadoAnimal estado,
        ...
        return ok(AnimalPageResponse.from(service.list(new AnimalFilter(estado, propiedadId, potreroId, loteId,
                categoria, sexo, search, page, size))), r);
    }

    @PostMapping("/animales")
    ApiResponse<AnimalResponse> create(@Valid @RequestBody CrearAnimalRequest b, HttpServletRequest r) {
    ...
    @GetMapping("/animales/{id}/historial")
    ApiResponse<List<AnimalEventResponse>> history(@PathVariable UUID id, HttpServletRequest r) {
    ...
    ApiResponse<TimelinePageResponse> timeline(@PathVariable UUID id,
                                               ...
                                               @RequestParam(defaultValue = "20") @Min(1) @Max(500) int size,
                                               HttpServletRequest r) {
        return ok(service.timeline(id, new EventoTimelineFilter(tipo, modulo, desde, hasta, usuarioId, page, size)), r);
    }

    @PatchMapping("/animales/{id}")
    ApiResponse<AnimalResponse> update(@PathVariable UUID id, @Valid @RequestBody ActualizarAnimalRequest b,
                                       HttpServletRequest r) {
        return ok(AnimalResponse.from(service.update(id, b.command())), r);
    }

    @PatchMapping("/animales/{id}/estado")
    ApiResponse<AnimalResponse> state(@PathVariable UUID id, @Valid @RequestBody CambiarEstadoAnimalRequest b,
                                      HttpServletRequest r) {
        return ok(AnimalResponse.from(service.changeState(id, b.estado(), b.motivo(), b.version())), r);
    }

    @GetMapping("/razas")
    ApiResponse<List<RazaResponse>> breeds(HttpServletRequest r) {
        return ok(service.breeds().stream().map(RazaResponse::from).toList(), r);
    }

    @GetMapping("/categorias-animal")
    ApiResponse<List<CategoriaAnimalResponse>> categories(HttpServletRequest r) {
        return ok(service.categories().stream().map(CategoriaAnimalResponse::from).toList(), r);
    }
