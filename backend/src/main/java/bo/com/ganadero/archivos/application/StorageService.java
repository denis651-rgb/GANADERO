...
@Service public class StorageService{
 private final FileStorageClient client;private final AppProperties properties;private final UserContext context;private final ImagenValidador validador;
 public StorageService(FileStorageClient client,AppProperties properties,UserContext context,ImagenValidador validador){this.client=client;this.properties=properties;this.context=context;this.validador=validador;}
 public StoredAvatar uploadAvatar(MultipartFile file){context.currentUser();byte[] content=bytes(file);validate(file,content);String extension=extension(file.getContentType());
  String path="avatares/"+UUID.randomUUID()+"."+extension;
  client.upload(path,content,file.getContentType());return new StoredAvatar(path,client.url(path));}
 public String signedAvatar(String path){if(path==null||!path.startsWith("avatares/")||path.contains(".."))throw new BusinessException(ErrorCode.PROPERTY_ACCESS_DENIED);return client.url(path);}
