...
@Service public class StorageService{
 private final SupabaseStorageClient client;private final AppProperties properties;private final UserContext context;
 public StorageService(SupabaseStorageClient client,AppProperties properties,UserContext context){this.client=client;this.properties=properties;this.context=context;}
 public StoredAvatar uploadAvatar(MultipartFile file){CurrentUser user=context.currentUser();validate(file);String extension=extension(file.getContentType());
  String path="empresas/"+user.empresaId()+"/usuarios/"+user.userId()+"/avatar/"+UUID.randomUUID()+"."+extension;
  try{client.upload(path,file.getBytes(),file.getContentType());return new StoredAvatar(path,client.signedUrl(path));}catch(IOException exception){throw new BusinessException(ErrorCode.STORAGE_FILE_INVALID);}}
 ...
 private void validate(MultipartFile file){if(file==null||file.isEmpty()||file.getSize()>properties.storage().maxBytes()||!properties.storage().allowedMimeTypes().contains(file.getContentType()))throw new BusinessException(ErrorCode.STORAGE_FILE_INVALID);
