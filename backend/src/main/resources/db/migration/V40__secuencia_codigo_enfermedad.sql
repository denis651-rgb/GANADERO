-- secuencia_codigo: se recrea (SQLite no permite ALTER de CHECK) solo para sumar 'ENFERMEDAD'
-- al conjunto ya vigente (PROPIEDAD,SECTOR,POTRERO,ANIMAL,LOTE,COMPRA,CATEGORIA); no se pierden contadores.
create table secuencia_codigo_nueva (
    ...
    constraint ck_secuencia_codigo_tipo check (tipo_entidad in ('PROPIEDAD','SECTOR','POTRERO','ANIMAL','LOTE','COMPRA','CATEGORIA','ENFERMEDAD')),
